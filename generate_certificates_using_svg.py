...
def replace_and_write(infile:Path,outfile:Path,motivation:str|None=None):
    """
    Replaces the keyhold words of the input file by the corresponding values.
    In the given example it assumes only the keyword "certificate_motivation" is included
    and it should be replaced by the motivation parameter.
    ...
    motivation : str | None, optional
        value of the placeholder "certificate_motivation" in the infile. If None is provided
        nothing will be substituted. By default None. 
    """
    with open(infile,'r') as F:
        txt = F.read()
    if motivation is not None: 
        txt = re.sub('certificate_motivation', motivation, txt)
    ...
    #
    #    if motivation is not None: 
    #       motivation_r = fullname.__repr__()[1:-1]
    #       txt = re.sub('certificate_motivation', motivation_r, txt)

    with open(outfile,'w') as F: 
        F.write(txt)

def replace_svg_in_texfile(infile:Path,outfile:Path,pngfile:str):
    """
    Replaces the 'background.png' of the input file by the pngfile and writes the file outfile.
    ...
    outfile : Path
        new file to be generated
    pngfile : str
        Value of the new png file used for background image.
    """
    with open(infile,'r') as F:
        txt = F.read()
    
    txt = re.sub('background.png', pngfile, txt)

    with open(outfile,'w') as F: 
        F.write(txt)

def create_certificate(motivation:str, odir:Path, overwrite=False, keeptex=False, keepsvg=False):
    """
    ...
    keepsvg : bool, optional
        If enabled it will keep a copy of the .svg file used to generate 
        the certificate, by default False.
    """
    base_svgfile = 'background.svg'
    base_texfile = 'main_base.tex'
    current_svgfile = Path('main.svg')
    ...
    replace_and_write(base_svgfile,current_svgfile,motivation=motivation)

    if keepsvg: 
        shutil.copy(current_svgfile,ofile.with_suffix('.svg'))

    inkscape_cmd = f'inkscape {current_svgfile} -o {current_svgfile.stem}.png --export-dpi {DPI}'
    subprocess.run(inkscape_cmd,
                   shell=True,
                   stdout=subprocess.DEVNULL,
                   stderr=subprocess.STDOUT)

    replace_svg_in_texfile(base_texfile, current_texfile, str(current_svgfile.with_suffix('.png')))
    ...
    Path(current_svgfile).unlink()
    Path(current_svgfile.with_suffix('.png')).unlink()
    shutil.rmtree(build)
...
if __name__ == '__main__':
    ...
    for rowid,row in df.iterrows():
        ...
        motivation = f'having the great name of {fullname}'
        if row['has_attended']:
            print('    attendance')
            create_certificate(motivation,
                               odir,
                               overwrite=overwrite,
                               keeptex=keeptex,
                               keepsvg=keepsvg)